...
+ (NSArray *)allDays:(ITTMonth *)month
{
    ...
    for (NSInteger i = 1; i <= previousInterval; i ++) {
        NSInteger dayNumber = month.previousMonth.lastDay.getDay - previousInterval + i;
        ITTDay *day = [[ITTDay alloc] initWithYear:[previousMonth getYear] month:[previousMonth getMonth] day:dayNumber hour:0 minute:0 second:0];
        day.color = PreviousMonthColor;
        [array addObject:day];
    }
    
    for (NSInteger i = 1; i <= month.days; i ++) {
    ...
    for (NSInteger i = 1; i <= nextInterval; i ++) {
        NSInteger dayNumber = i;
        ITTDay *day = [[ITTDay alloc] initWithYear:[nextMonth getYear] month:[nextMonth getMonth] day:dayNumber hour:0 minute:0 second:0];
